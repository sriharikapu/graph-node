    ...
    T::Out: Send,
{
    fn net_identifiers(
        ...
        let web3 = self.web3.clone();
        let net_version_future = with_retry_max_retry(10, move || {
            web3.net().version().map_err(SyncFailure::new).from_err()
        });

        let web3 = self.web3.clone();
        let gen_block_hash_future = with_retry_max_retry(10, move || {
            web3.eth()
