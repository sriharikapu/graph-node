    ...
    S: Store,
{
    ...
    fn remove(
        ...
        let names_and_id_opt_result = self
            .store
            .read_subgraph_name(name_or_id.clone())
            .map_err(SubgraphProviderError::Unknown)
            .and_then(move |subgraph_id_opt_from_name_opt| {
                match subgraph_id_opt_from_name_opt {
                    ...
                    None => {
                        // name_or_id must be an ID
                        let id = name_or_id;

                        // Look up names that match
                        self.store
                            .find_subgraph_names_by_id(id.clone())
                            .map_err(SubgraphProviderError::Unknown)
                            .and_then(move |names| {
                                if names.is_empty() {
                                    Err(SubgraphProviderError::NotFound(id))
                                } else {
                                    Ok((names, Some(id)))
                                }
                            })
